...
class BaseQuantizer(nn.Module):
    ...
    def __init__(self, quant_config):
        ...
        self.running_stat = quant_config.get(
            "running_stat", False
        )  # INFO: use running_stat to accmulate quant_params, used in activation
    ...
    def init_quant_params(self, x: torch.Tensor, per_group: bool = False, momentum=False, n_bits=None):
        ...
        if self.scale_method == "min_max":
            ...
            if delta.min() < eps:  # if contain small range
                ...
                warnings.warn(
                    'For layer "{}", quant stept size close to zero, set as EPS:{}'.format(self.module_name, eps)
                )
            ...
            lp_loss_ranged = lp_loss(x_ranged, x_q, p=2.0, reduction="none", n_batch=n_step)  # [N_bs]
        ...
        if self.round_mode == "learned_hard_sigmoid":
            ...
            delta_shape = [1] * len(
                x.shape
            )  # temporarily reshape self.delta to fit current x shape, x is reshaped into 2-dim, note that the x_shape are longer than x.shape
        ...
        if per_group == "token":
            ...
            assert isinstance(delta, torch.Tensor), "during init, delta should be a tensor, instead of type: {}".format(
                type(self.delta)
            )
        ...
        if self.delta_list is None:  # init the delta_list according to actual delta/zp shape
            self.delta_list = (
                torch.zeros([self.n_bitwidth, self.n_timestep] + list(delta.shape), dtype=delta.dtype)
                .fill_(-1.0)
                .to(x.device)
            )
            self.zero_point_list = (
                torch.zeros([self.n_bitwidth, self.n_timestep] + list(zero_point.shape), dtype=zero_point.dtype)
                .fill_(-1.0)
                .to(x.device)
            )
        self.delta_list[
            i_bitwidth, self.cur_timestep_id
        ] = delta  # when timestep_wise=False, self.cur_timestep_id=-1, index the only one
