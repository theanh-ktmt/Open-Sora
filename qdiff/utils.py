...
class LossFunction:
    ...
    def __init__(
        ...
        lambda_coeff: float = 1.0,  # the coeff between two loss
        iters: int = 2000,
        b_range: tuple = (10, 2),
        ...
        self.temp_decay = LinearTempDecay(
            iters, rel_start_decay=warmup + (1 - warmup) * decay_start, start_b=b_range[0], end_b=b_range[1]
        )
        self.count = 0
    ...
    def __call__(self, pred, tgt, grad=None):
        ...
        if self.use_round_loss:
            if self.count < self.loss_start or self.round_loss_type == "none":
                ...
                if self.module_type == "layer":
                    ...
                    round_loss += self.lambda_coeff * (1 - ((round_vals - 0.5).abs() * 2).pow(b)).sum()
        ...
        if self.count % 100 == 0:
            ...
            logger.info(
                "Total loss:\t{:.6f} (rec:{:.6f}, round:{:.6})\tb={:.2f}\tcount={}".format(
...
def save_in_out_data(
    ...
    for i in trange(iters):
        if model_type == "sdxl":
            ...
            tmp_kwargs = {
                "fcf": 4.5,
                "data_info": {
                    "img_hw": torch.tensor([[1024.0, 1024.0]], device=device),
            ...
            if len(cur_inp) > 2:
                ...
                if not split_save_attn:
                    ...
                    cached_inps[0].index_copy_(
                        0, torch.arange(i * cur_x.shape[0], (i + 1) * cur_x.shape[0]), cur_x.cpu()
                    )
                    cached_inps[1].index_copy_(
                        0, torch.arange(i * cur_t.shape[0], (i + 1) * cur_t.shape[0]), cur_t.cpu()
                    )
        else:
            if not split_save_attn:
                ...
                cached_inps.index_copy_(
                    0, torch.arange(i * cur_inp.shape[0], (i + 1) * cur_inp.shape[0]), cur_inp.cpu()
                )
    ...
    if isinstance(cached_inps, list):
        for i in range(len(cached_inps)):
            logger.info(f"in {i} shape: {cached_inps[i].shape}") if cached_inps[i] is not None else logger.info(
                f"in {i} : None"
            )
    else:
...
class GetLayerInOut:
    def __init__(
        ...
        layer: Union[QuantLayer, BaseQuantBlock],
        model_type="sd",
        previous_layer_quantized=False,
    ):
    ...
    def __call__(self, x, timesteps, context=None, added_conds=None, mask=None):
        ...
        with torch.no_grad():
            ...
            if self.model_type == "pixart":
                tmp_kwargs = {
                    "fcf": 4.5,
                    "data_info": {
                        "img_hw": torch.tensor([[1024.0, 1024.0]], device="cuda"),
        ...
        if (
            len(self.data_saver.input_store) > 1
            and len(self.data_saver.input_store) < 7
...
class GetLayerGrad:
    def __init__(
        self, model: QuantModel, layer: Union[QuantLayer, BaseQuantBlock], device: torch.device, act_quant: bool = False
    ):
        self.model = model
        self.layer = layer
        self.device = device
