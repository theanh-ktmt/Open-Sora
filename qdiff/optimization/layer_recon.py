...
from qdiff.utils import LossFunction, save_grad_data, save_in_out_data

logger = logging.getLogger(__name__)


def layer_reconstruction(
    model: QuantModel, layer: QuantLayer, calib_data: torch.Tensor, config, param_types, opt_target
):
    ...
    if len(calib_data) == 4:
        if config.model.model_type == "pixart":
            # pixart has {xs, t, condition, masks}
            cached_inps, cached_outs = save_in_out_data(model, layer, calib_data, config, model_type="pixart")
        elif config.model.model_type == "sdxl":
    ...
    if not use_grad:
        ...
        cached_grads = save_grad_data(
            model, layer, calib_data, act_quant=False, batch_size=batch_size
        )  # TODO: reduce act_quant
