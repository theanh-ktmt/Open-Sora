...
def block_reconstruction(
    model: QuantModel, block: BaseQuantBlock, calib_data: torch.Tensor, config, param_types, opt_target
):
    ...
    if not use_grad:
        ...
        cached_grads = save_grad_data(
            model, block, calib_data, act_quant=False, batch_size=batch_size
    ...
    sample_idxs = torch.randint(
        low=0, high=cached_inps[0].shape[0], size=(iters, batch_size), device=cached_inps[0].device
    )
    ...
    if not act_quant:
        ...
        optimizer = torch.optim.Adam(opt_params, lr=1.0e-2)
