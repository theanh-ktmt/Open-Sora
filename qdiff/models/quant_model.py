...
from qdiff.models.quant_block import BaseQuantBlock, QuantTransformerBlock, TransformerBlock, get_specials
from qdiff.models.quant_layer import QuantLayer
from qdiff.models.stdit_quant_layer import QuantCrossAttnLinear, QuantSpatialAttnLinear, QuantTemporalAttnLinear
from qdiff.quantizer.base_quantizer import ActQuantizer, BaseQuantizer, StraightThrough, WeightQuantizer
                ...
                if ".attn." in full_name:
                    if self.model_type == "opensora":
                        setattr(
                            module,
                            name,
                            QuantSpatialAttnLinear(child_module, weight_quant_params, act_quant_params),
                        )
                    elif self.model_type == "pixart":
                        setattr(
                            module,
                            name,
                            QuantAttnLinearImg(child_module, weight_quant_params, act_quant_params),
                        )
                    ...
                    if self.model_type == "opensora":
                        setattr(
                            module,
                            name,
                            QuantCrossAttnLinear(child_module, weight_quant_params, act_quant_params),
                        )
                    elif self.model_type == "pixart":
                        setattr(
                            module,
                            name,
                            QuantCrossAttnLinearImg(child_module, weight_quant_params, act_quant_params),
                        )
                elif "attn_temp" in full_name:
                    setattr(
                        module,
                        name,
                        QuantTemporalAttnLinear(child_module, weight_quant_params, act_quant_params),
                    )
                else:
                    setattr(
                        module,
                ...
                if load_buffer_only:
                    ...
                    for name, quant_params in quant_params_dict[module_.module_name][
                        ...
                        setattr(
                            module_,
                            name,
                            quant_params.to(dtype) if quant_params is not None else None,
                        )
                else:
                    # set buffer
