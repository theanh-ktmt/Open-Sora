...
from qdiff.quantizer.base_quantizer import ActQuantizer, StraightThrough, WeightQuantizer
    ...
    def forward(self, input: torch.Tensor, scale: float = 1.0, split: int = 0, smooth_quant_enable: bool = False):
        ...
        if self.smooth_quant:
            if self.channel_wise_scale_type == "dynamic":
                channel_wise_scale = input.abs().max(dim=-2)[0].pow(self.smooth_quant_alpha).mean(
                    dim=0, keepdim=True
                ) / self.weight.abs().max(dim=0)[0].pow(1 - self.smooth_quant_alpha)
                ...
                if self.smooth_quant_running_stat:
                    ...
                    if self.act_quantizer.act_scale is None:
                        ...
                        self.act_quantizer.act_scale = (
                            self.act_quantizer.act_scale * self.smooth_quant_momentum
                            + cur_act_scale * (1 - self.smooth_quant_momentum)
                        )
                else:
                    assert self.act_quantizer.act_scale is not None
                channel_wise_scale = self.act_quantizer.act_scale.pow(self.smooth_quant_alpha) / self.weight.abs().max(
                    dim=0
                )[0].pow(1 - self.smooth_quant_alpha)
            else:
