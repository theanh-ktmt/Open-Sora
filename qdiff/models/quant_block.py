...
from qdiff.quantizer.base_quantizer import ActQuantizer, StraightThrough
...
def set_qprocessor(
    attn: Union["QuantAttention", "Attention"], qprocessor: "QuantAttnProcessor", _remove_lora: bool = False
) -> None:
    r"""
...
class QuantAttention(BaseQuantBlock):
    def __init__(
        ...
        qprocessor: Optional["QuantAttnProcessor"] = None,
    ):
        super().__init__(act_quant_params)
        self.attn = attn
...
class QuantAttnProcessor:
    ...
    def __call__(
        ...
        if is_cross_attn and (
            (attn.to_k.act_quant or attn.to_k.weight_quant) or (attn.to_v.act_quant or attn.to_v.weight_quant)
        ):
        ...
        if not SPLIT_FIRST_TOKEN:
            ...
            key_first_token = attn.to_k.fwd_func(
                encoder_hidden_states[:, 0, :].unsqueeze(1),
                attn.to_k.org_weight,
                attn.to_k.org_bias,
                **attn.to_k.fwd_kwargs,
            )
            key = torch.cat([key_first_token, key], dim=1)

            value = attn.to_v(encoder_hidden_states[:, 1:, :], *args)
            value_first_token = attn.to_v.fwd_func(
                encoder_hidden_states[:, 0, :].unsqueeze(1),
                attn.to_v.org_weight,
                attn.to_v.org_bias,
                **attn.to_v.fwd_kwargs,
            )
