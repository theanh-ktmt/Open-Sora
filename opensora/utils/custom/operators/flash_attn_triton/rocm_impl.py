...
def get_cdna_autotune_configs():
    return [
        triton.Config(
            {"BLOCK_M": 128, "BLOCK_N": 128, "waves_per_eu": 2, "PRE_LOAD_V": False}, num_stages=1, num_warps=4
        ),
        triton.Config(
        ...
        triton.Config(
            {"BLOCK_M": 128, "BLOCK_N": 32, "waves_per_eu": 2, "PRE_LOAD_V": False}, num_stages=1, num_warps=4
        ),
        # Fall-back config.
        triton.Config(
            {"BLOCK_M": 16, "BLOCK_N": 16, "waves_per_eu": 1, "PRE_LOAD_V": False}, num_stages=1, num_warps=4
        ),
...
def get_rdna_autotune_configs():
    return [
        triton.Config(
            {"BLOCK_M": 32, "BLOCK_N": 32, "waves_per_eu": 4, "PRE_LOAD_V": False}, num_stages=1, num_warps=2
        ),
        triton.Config(
            {"BLOCK_M": 32, "BLOCK_N": 32, "waves_per_eu": 2, "PRE_LOAD_V": False}, num_stages=1, num_warps=2
        ),
        triton.Config(
            {"BLOCK_M": 32, "BLOCK_N": 16, "waves_per_eu": 4, "PRE_LOAD_V": False}, num_stages=1, num_warps=2
        ),
        triton.Config(
            {"BLOCK_M": 32, "BLOCK_N": 16, "waves_per_eu": 2, "PRE_LOAD_V": False}, num_stages=1, num_warps=2
        ),
        triton.Config(
            {"BLOCK_M": 16, "BLOCK_N": 16, "waves_per_eu": 4, "PRE_LOAD_V": False}, num_stages=1, num_warps=2
        ),
        triton.Config(
            {"BLOCK_M": 16, "BLOCK_N": 16, "waves_per_eu": 2, "PRE_LOAD_V": False}, num_stages=1, num_warps=2
        ),
        # Fall-back config.
        triton.Config(
            {"BLOCK_M": 16, "BLOCK_N": 16, "waves_per_eu": 1, "PRE_LOAD_V": False}, num_stages=1, num_warps=2
        ),
    ], ["IS_CAUSAL", "dropout_p", "MAX_SEQLENS_Q", "MAX_SEQLENS_K", "ACTUAL_BLOCK_DMODEL", "VARLEN", "HQ", "HK"]
    ...
    def forward(ctx, q, k, v, o, metadata):
        ...
        if metadata.return_encoded_softmax:
            encoded_softmax = torch.zeros(
                (q.shape[0], q.shape[1], q.shape[2], k.shape[2]), device=q.device, dtype=torch.float32
            )
        else:
...
def test_op_fwd(Z, HQ, HK, N_CTX_Q, N_CTX_K, D_HEAD, causal, use_alibi, layout, dtype=torch.float16):
    ...
    if use_alibi:
        ...
        alibi_slopes = torch.tensor(
            [2 ** (-8 / HQ * i) for i in range(1, HQ + 1)], dtype=torch.float32, device="cuda"
        ).repeat(Z, 1)
...
def test_op_bwd(
    Z, H, N_CTX, D_HEAD, qseqlen_not_equal_kseqlen, causal, torch_sdpa_test, use_alibi, dtype=torch.float16
):
    pytest.skip()
    torch.manual_seed(20)
    ...
    if use_alibi and not torch_sdpa_test:
        # for n heads the set of slopes is the geometric sequence that starts 2^(-8/n)
        alibi_slopes = torch.tensor(
            [2 ** (-8 / H * i) for i in range(1, H + 1)], dtype=torch.float32, device="cuda"
        ).repeat(Z, 1)
    ...
    if torch_sdpa_test:
        ref_out, ref_softmax = torch.ops.aten._scaled_dot_product_attention_math(
            q, k, v, dropout_p=dropout_p, is_causal=causal, scale=sm_scale, dropout_mask=None
        )
...
def main():
    ...
    assert args.dtype in arg_to_torch_dtype, "Only fp16, bf16 and f32 types currently supported."
